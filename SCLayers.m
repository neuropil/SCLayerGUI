function varargout = SCLayers(varargin)
% SCLAYERS MATLAB code for SCLayers.fig
%      SCLAYERS, by itself, creates a new SCLAYERS or raises the existing
%      singleton*.
%
%      H = SCLAYERS returns the handle to a new SCLAYERS or the handle to
%      the existing singleton*.
%
%      SCLAYERS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SCLAYERS.M with the given input arguments.
%
%      SCLAYERS('Property','Value',...) creates a new SCLAYERS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SCLayers_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SCLayers_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help SCLayers

% Last Modified by GUIDE v2.5 15-Mar-2014 14:44:28

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @SCLayers_OpeningFcn, ...
    'gui_OutputFcn',  @SCLayers_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before SCLayers is made visible.
function SCLayers_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to SCLayers (see VARARGIN)

% Choose default command line output for SCLayers
...
% UIWAIT makes SCLayers wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = SCLayers_OutputFcn(hObject, eventdata, handles)
